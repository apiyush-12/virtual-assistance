...
import bg from "../assets/auth1.jpg"
...
function signIn() {
  ...
  return (
    ...
    <form className='w-[90%] h-125 max-w-125 bg-[#00000000] backdrop-blur shadow-lg shadow-red-950 rounded-lg flex flex-col justify-center items-center gap-5 px-5' onSubmit={handleSignIn}>
        <h1 className='text-white text-[30px] font-semibold mb-7.5'>Sign In to <span className='text-blue-500'>Virtual Assistant</span></h1>
        ...
        <p className='text-white text-[18px]' onClick={()=>navigate("/SignUp")}>Don't have an account? <span className='text-blue-500 hover:underline cursor-pointer'>Sign Up</span></p>
    </form>
    </div>
  )
}
